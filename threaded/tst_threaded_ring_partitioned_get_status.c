...
int tst_threaded_ring_partitioned_get_status_init(struct tst_env *env)
{
  int comm_rank;
...
static int wait_for_partition(MPI_Request *recv_request, int partition_num, useconds_t backoff_time)
{
  ...
  do
  {
    // 
    int completion_flag;
    MPI_CHECK(MPI_Request_get_status(*recv_request, &completion_flag, MPI_STATUS_IGNORE));

    MPI_CHECK(MPI_Parrived(*recv_request, partition_num, &flag));

    // check that completion_flag => flag
    if (completion_flag && !flag) {
      return 1;
    }
...
int tst_threaded_ring_partitioned_get_status_run(struct tst_env *env)
{
  int comm_size;
  int comm_rank;
  int send_to;
  ...
  else if (tst_comm_getcommclass(env->comm) & TST_MPI_COMM_SELF)
  {
    ...
    ERROR(EINVAL, "tst_threaded_ring_get_status_partitioned cannot run with this kind of communicator");
  ...
  if (comm_rank == TST_RANK_MASTER)
  {
    ...
    if (send_partition_num >= 0 && send_partition_num < num_send_partitions)
    {
      // 
      int flag;
      MPI_CHECK(MPI_Request_get_status(*send_request, &flag, MPI_STATUS_IGNORE));
      if (flag) {
        errors++;
      }
...
int tst_threaded_ring_partitioned_get_status_cleanup(struct tst_env *env)
{
  int thread_num = tst_thread_get_num();
  int num_worker_threads = tst_thread_num_threads();
