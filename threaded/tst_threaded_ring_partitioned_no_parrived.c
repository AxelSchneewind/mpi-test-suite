/*
 * File: tst_threaded_ring_partitioned_no_parrived.c
 *
 * Functionality:
...
int tst_threaded_ring_partitioned_no_parrived_init(struct tst_env *env)
{
  int comm_rank;
...
int tst_threaded_ring_partitioned_no_parrived_run(struct tst_env *env)
{
  int comm_size;
  int comm_rank;
  int send_to;
  ...
  else if (tst_comm_getcommclass(env->comm) & TST_MPI_COMM_SELF)
  {
    ...
    ERROR(EINVAL, "tst_threaded_ring_partitioned_no_parrived cannot run with this kind of communicator");
...
int tst_threaded_ring_partitioned_no_parrived_cleanup(struct tst_env *env)
{
  int thread_num = tst_thread_get_num();
  int num_worker_threads = tst_thread_num_threads();
